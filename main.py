...
import random
import datetime
from config import telegram_token, open_weather_token, exchange_rate_token
...
async def start_command(message: types.Message):
    await message.answer(text="<b>Добро пожаловать в мой телеграм бот!</b>",
                         parse_mode='HTML',
                         reply_markup=kb)
...
async def send_exchange(message: types.Message):
    ...
    exchange_flag = True
    await message.answer(text='Введите через пробел коды двух валют (например: "USD EUR")',
                         reply_markup=kb_back)
